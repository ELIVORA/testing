...
interface EnterpriseMockInterviewStudioProps {
  profile: {
    fullName: string;
    university: string;
...
export function EnterpriseMockInterviewStudio({ profile, resumeFileName, launchMode, resumeAnalysisData }: EnterpriseMockInterviewStudioProps) {
  ...
  const behaviorMetricsRef = useRef(liveBehaviorMetrics);
  ...
  // Persist lightweight camera/behavior telemetry so the session report and
  // long-term Candidate Memory can use real session evidence.
  useEffect(() => { behaviorMetricsRef.current = liveBehaviorMetrics; }, [liveBehaviorMetrics]);
  useEffect(() => {
    if (view !== "active" || !session || !cameraGranted) return;
    const timer = window.setInterval(() => {
      const metrics = behaviorMetricsRef.current;
      api.post(`/v1/camera/session/${session.session_id}/telemetry`, {
        confidence: metrics.confidence,
        clarity: metrics.clarity,
        communication: metrics.communication,
        posture: metrics.completeness,
        eye_contact: metrics.confidence,
        speaking_pace: metrics.speakingPace
      }).catch(() => undefined);
    }, 5000);
    return () => window.clearInterval(timer);
  }, [view, session?.session_id, cameraGranted]);
                ...
                <div className="p-6 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-850 rounded-3xl space-y-6">
                  <div>
                    ...
                    <div className="mt-3.5 space-y-3">
                      {(Array.isArray(session.overall_report.suggested_improvements) ? session.overall_report.suggested_improvements : Object.values(session.overall_report.suggested_improvements || {})).map((imp: any, idx: number) => (
